...
ReconstructionResult saveColoredPointCloudPLY_OURS(
    ...
    // ---- Input checks ----
    if (disparity_f.empty()) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: disparity_f is empty");
    }
    if (img_left_color.empty()) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: img_left_color is empty");
    }
    if (disparity_f.type() != CV_32F) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: disparity_f must be CV_32F");
    }
    if (img_left_color.type() != CV_8UC3) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: img_left_color must be CV_8UC3 (BGR)");
    }
    if (K.empty() || K.rows != 3 || K.cols != 3 || K.type() != CV_64F) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: K must be 3x3 CV_64F");
    }
    ...
    const int W = std::min(disparity_f.cols, img_left_color.cols);
    if (H <= 0 || W <= 0) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: invalid overlap size");
    }
    ...
    // ===== ONLY DIFFERENCE: OURS baseline computation =====
    const double baseline_used_mm = baseline_mm * params.baseline_scale;

    if (fx <= 0.0 || fy <= 0.0 || baseline_used_mm <= 0.0) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: invalid intrinsics or baseline");
    }
    ...
    for (int y = 0; y < H; ++y) {
        ...
        for (int x = 0; x < W; ++x) {
            ...
            const float Z_mm = (float)(fx * baseline_used_mm / (double)d_geom);
            const float Z_m  = Z_mm * 0.001f;
            if (!std::isfinite(Z_m)) continue;
            if (Z_m < Z_MIN_M || Z_m > Z_MAX_M) continue;
    ...
    std::ofstream ofs(ply_path);
    if (!ofs.is_open()) {
        throw std::runtime_error("saveColoredPointCloudPLY_OURS: failed to open " + ply_path);
    }
